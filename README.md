...
- **高还原钉钉界面**：状态栏、双行导航栏（标题 + 副标题 + 功能图标）、消息区、底部功能区（群签到 / 水印拍照 / 输入框 / 工具栏）均按钉钉真实样式还原
- **聊天记录编辑**：左侧编辑、右侧实时预览，所见即所得
- **昵称与已读回执**：对方消息上方显示灰色昵称，自己的消息下方可显示"全部已读"（可开关）
- **成员管理**：添加、删除、重命名成员，支持自定义头像
  - 文字头像：钉钉风格浅蓝底 + 蓝字，自动取名字前两字，圆角方形
  - 图片头像：上传本地图片，自动居中裁剪并压缩为 96×96 圆形头像
- **消息编辑**：选发送者（任意成员或"我"）、输入内容，支持上移、下移、删除
- **消息类型**：文本消息、时间分割、系统消息（如"xxx 加入群聊"）
- **会话设置**：自定义群名、导航栏副标题、我的昵称、我的头像
- **本地持久化**：所有编辑内容自动保存在浏览器本地，刷新页面不丢失
...
- 编辑数据与头像（压缩后的 base64）自动保存在浏览器 localStorage 中，仅存本机，不会上传到任何服务器
